            ...
            @if (isset($attributes[0]))
                @foreach ($attributes as $key => $attribute)
                    <option value="{{ $attribute->id }}"
                        {{ in_array($attribute->id, $categoryAttributes) && in_array($attribute->id, $productAttributes) ? 'selected' : '' }}
                        {{ !in_array($attribute->id, $categoryAttributes) ? 'hidden' : '' }}>{{ $attribute->name }}
                        </attribute>
                @endforeach
            @endif
        </select>
                                ...
                                @if (isset($attribute->options[0]))
                                    @foreach ($attribute->options as $key => $option)
                                        <option value="{{ $option->id }}"
                                            {{ in_array($option->id, $categoryAttributeOptions) && $product->attributes->where('attribute_option_id', $option->id)->count() > 0 ? 'selected' : '' }}
                                            {{ !in_array($option->id, $categoryAttributeOptions) ? 'hidden' : '' }}>
                                            {{ $option->name }}</option>
                                    @endforeach
                                @endif
                            </select>

    ...
    <div class="col-md-2">
        <div class="form-group">
            <div class="form-line">
                ...
                {!! Form::Select('status',array('pending'=>'Pending','approved'=>'Approved'),Request::old('status'),['id'=>'status', 'class'=>'form-control select2']) !!}
                {!! $errors->first('status') !!}
            </div>
        </div>
    </div>
        ...
        <select name="productAttributes[]" id="attributes"
            ...
            @if(isset($attributes[0]))
                @foreach($attributes as $key => $attribute)
                    <option
                        value="{{ $attribute->id }}" {{ in_array($attribute->id, $categoryAttributes) && in_array($attribute->id, $productAttributes) ? 'selected' : '' }} {{ !in_array($attribute->id, $categoryAttributes) ? 'hidden' : '' }}>{{ $attribute->name }}</attribute>
                @endforeach
            @endif
        </select>
    </div>
            ...
            @if(isset($attributes[0]))
                @foreach($attributes as $key => $attribute)
                    <div class="col-md-3 mt-1 attributes attribute-{{ $attribute->id }}"
                         style="display: {{ in_array($attribute->id, $categoryAttributes) && in_array($attribute->id, $productAttributes) ? 'block' : 'none' }}">
                            ...
                            <select name="attribute_option_id[{{ $attribute->id }}]" id="attribute-{{ $attribute->id }}"
                                ...
                                @if(isset($attribute->options[0]))
                                    @foreach($attribute->options as $key => $option)
                                        <option
                                            value="{{ $option->id }}" {{ in_array($option->id, $categoryAttributeOptions) && $product->attributes->where('attribute_option_id', $option->id)->count() > 0 ? 'selected' : '' }}  {{ !in_array($option->id, $categoryAttributeOptions) ? 'hidden' : '' }}>{{ $option->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>

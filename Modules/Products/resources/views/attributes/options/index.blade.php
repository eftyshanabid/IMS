                                            ...
                                            <div class="col-md-4">
                                                        ...
                                                        @if (isset($attributes[0]))
                                                            @foreach ($attributes as $key => $attribute)
                                                                <option value="{{ $attribute->id }}"
                                                                    {{ request()->get('attribute_id') == $attribute->id ? 'selected' : '' }}>
                                                                    [{{ $attribute->code }}
                                                ...
                                                @if (request()->has('attribute_id') && request()->get('attribute_id') > 0)
                                                    @can('attribute-edit')
                                                        <a class="text-primary pl-1"
                                                         ...
                                                         'Attribute', '{{ url('admin/attributes/' . request()->get('attribute_id') . '/edit') }}');"
                                                ...
                                                @can('attribute-create')
                                                    <a class="text-success"
                                                        onclick="openModal('New Attribute', '{{ url('admin/attributes/create') }}');"
                                                        style="font-size: 12px"><i class="mdi mdi-plus me-1"></i>&nbsp;New
                                                        Attribute</a>
                                                @endcan
                                            </div>
                                        </div>
                                ...
                                <div class="col-xl-4">
                                    <div class="text-xl-end mt-xl-0 mt-2">
                                        @if (isOptionPermitted('attribute-option-create'))
                                            <a href="{{ url('admin/attribute-options/create?attribute_id=' . request()->get('attribute_id')) }}"
                                                class="btn btn-info mb-2 me-2" data-toggle="tooltip"
                                                title="New Attribute Option">
                                                <i class="mdi mdi-plus me-1"></i>{{ translate('New Attribute Option') }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
